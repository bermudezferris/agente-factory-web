...
import { takeConversation } from "@/services/conversations/handoff-service";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    await takeConversation(dependencies.repository, id);
    return Response.json({ status: "HUMAN_ACTIVE" });
  } catch (error) {
